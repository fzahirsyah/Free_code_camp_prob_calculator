...
class UnitTests(unittest.TestCase):
    ...
    def test_prob_experiment(self):
        ...
        probability = prob_calculator.experiment(hat=hat, expected_ball={"blue":2,"green":1}, number_ball_drawn=4, number_experiments=1000)
        actual = probability
        expected = 0.272
        self.assertAlmostEqual(actual, expected, delta = 0.01, msg = 'Expected experiment method to return a different probability.')
        hat = prob_calculator.hat(yellow=5,red=1,green=3,blue=9,test=1)
        probability = prob_calculator.experiment(hat=hat, expected_ball={"yellow":2,"blue":3,"test":1}, number_ball_drawn=20, number_experiments=100)
        actual = probability
        expected = 1.0
        self.assertAlmostEqual(actual, expected, delta = 0.01, msg = 'Expected experiment method to return a different probability.')
